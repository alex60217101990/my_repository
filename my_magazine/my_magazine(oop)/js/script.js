        ...
        arr_of_menu_div[q].addEventListener("click", function () {
                ...
                for (let j = 0; j < array_products.length; j++) {
                    ...
                    if (array_products[j].catId.name === arr_of_menu_div[q].textContent) {                /*!!!!!!!!!!!!!*/
                        ...
                        /*jQuery*/
                        let buy=$('<img/>');
                        buy.addClass( "buy" ).attr('src', 'images/buy.png').css('visibility', 'visible').appendTo( div2 )
                            .on({
                                click: function (event) {
                                  /* arr_buy.push($(this).parent());*/
                                    arr_buy.push($(this).parent());
                                    catName.push(array_products[j].name);
                                    catName.push(array_products[j].price);
                                    catName.push(array_products[j].image);
                                    catName.push(array_products[j].dopInfo);
                                    catName.push(array_products[j].catId.name);
                                console.log(arr_buy)}
                            });
                        /*jQuery*/
                        ...
                        for(let i=0; i<array_products.length; i++){
                            $('#closeI'+i).mouseout(function () {
                                if($(this).css('opacity')!='0')
                                    $(this).css('opacity','0');
                            });
                            $('#closeI'+i).mouseover(function () {
                                if($(this).css('opacity')=='0')
                                    $(this).css('opacity','1');
                            });
                        }
